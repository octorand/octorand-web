...
@Component({
  selector: 'app-play-games-prediction-algo',
  templateUrl: './prediction.page.html',
  styleUrls: ['./prediction.page.scss'],
})
export class PlayGamesPredictionAlgoPage implements OnInit, OnDestroy {
   ...
   * Games list
   */
  gamesList: Array<PredictionAlgoModel> = [];
  ...
  loadGamesList() {
    ...
    Promise.all(promises).then((values: Array<any>) => {
      ...
      let gamesList = this.PredictionAlgoService.list(applications);
  ...
  refreshGamesList() {
    if (!this.actions.loadingGames) {
      for (let i = 0; i < this.gamesList.length; i++) {
        this.gamesList[i] = this.PredictionAlgoService.refresh(this.gamesList[i]);
      }
